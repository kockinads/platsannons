...
@app.get("/api/health")
async def health():
    # extra liten koll för UI: hur många poster finns?
    with get_session() as db:
        cnt = db.query(JobPosting).count()
    return {"ok": True, "count": cnt}

# --- Admin: triggera insamling manuellt --------------------------------------

@app.post("/api/admin/harvest", dependencies=[Depends(require_auth)])
async def admin_harvest():
    await run_harvest()
    # enkel rapport
    with get_session() as db:
        total = db.query(JobPosting).count()
    return {"ok": True, "total_jobs": total}

# --- API: jobb-lista (med read-time dedupe-skydd) -----------------------------

def _key_for_dedupe(title_norm: str, employer_norm: str, city_norm: str, url: str) -> str:
    from .crud import _normalize_url  # återanvänd
    return "|".join([title_norm or "", employer_norm or "", city_norm or "", _normalize_url(url)])
...
async def list_jobs(
    ...
    if hide_recruiters:
        rows = [r for r in rows if not is_recruiter(r.employer, r.description or "")]

    # Read-time dedupe: välj senaste per (title_norm, employer_norm, city_norm, url_norm)
    best: Dict[str, JobPosting] = {}
    for r in rows:
        key = _key_for_dedupe(r.title_norm or "", r.employer_norm or "", r.city_norm or "", r.url or "")
        prev = best.get(key)
        if not prev or (r.published_at or datetime.min) > (prev.published_at or datetime.min):
            best[key] = r
...
async def export_leads(db: Session = Depends(get_session)):
    ...
    for lead, job in rows:
        writer.writerow([lead.tier, lead.notes, job.title, job.employer, job.city, job.region, job.published_at.isoformat() if job.published_at else "", job.url])
